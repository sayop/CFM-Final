...
def plotUmagContour(x,y,U,V,nIter):
...
def plotContour(x,y,phi,nIter,field):
   from matplotlib import mlab, cm
   cmap = cm.PRGn

   pltFile = 'contour_%s_%5.5d.png' % (field,int(nIter))
   x = np.asarray(x)
   y = np.asarray(y)
   #U = np.swapaxes(U,1,0)
   #V = np.swapaxes(V,1,0)
   ...
   #plt.contour(xi,yi,zi,levels)
   plt.imshow(phi, vmin=phiMin, vmax=phiMax, extent=[x.min(), x.max(), y.min(), y.max()])
   plt.colorbar()
